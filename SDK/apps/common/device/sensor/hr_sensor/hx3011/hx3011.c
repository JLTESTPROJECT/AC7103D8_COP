...
void gsensor_write_cbuf(short frame, short *buf)
{
    ...
    if (gsensor_cbuf == NULL || w_size == 0) {
        return ;
    }

    //如果cbuf满了丢掉旧数据，写入新数据，以保证数据最新
    u32 e_size = gsensor_cbuf->total_len - gsensor_cbuf->data_len;
    if (e_size <= w_size) {
        u32 p_size = w_size - e_size;
        p_size = p_size < 2 * 3 ? 2 * 3 : p_size;
        cbuf_read_alloc_len_updata(gsensor_cbuf, p_size);
    }
...
bool hx3011_write_reg(uint8_t addr, uint8_t data)
{
    if (hrsensor_write_nbyte(0x88, addr, &data, 1) != 1) {
        log_e("hx3011_write_reg error! addr=%02x data=%02x\n", addr, data);
    }
    // hx3011_delay(1);
    return true;
}
...
uint8_t hx3011_read_reg(uint8_t addr)
{
    ...
    if (hrsensor_read_nbyte(0x89, addr, &data_buf, 1) != 1) {
        log_e("hx3011_read_reg error! addr=%02x\n", addr);
    }
    return data_buf;
}
...
bool hx3011_brust_read_reg(uint8_t addr, uint8_t *buf, uint8_t length)
{
    if (hrsensor_read_nbyte(0x89, addr, buf, length) != length) {
        log_e("hx3011_brust_read_reg error! addr=%02x length=%02x\n", addr, length);
    }
    return true;
}
...
void hx3011_ppg_timer_cfg(bool en)    //320ms
{
    ...
    TYHX_LOG("hx3011_ppg_timer_cfg %d handle=%d\n", en, timer_handle);
}
...
void hx3011_agc_timer_cfg(bool en)
{
    ...
    TYHX_LOG("hx3011_agc_timer_cfg %d handle=%d\n", en, timer_handle);
...
bool hx3011_chip_check(void)
{
    ...
    TYHX_LOG("hx3011_chip_check fail\r\n");
    return false;
}
...
void read_data_packet(int32_t *ps_data)  // 3011
{
...
bool hx3011_init(WORK_MODE_T mode)
{
    ...
    case FT_LEAK_LIGHT_MODE:
        if (!hx3011_chip_check()) {
            AGC_LOG("hx3011 check id failed!\r\n");
            return false;
        }
...
#else
    if (hx3011_hrs_read(&ppg_s_dat) == 0) {
        return;
    }
    gsensor_read_cbuf();
...
#else
    if (hx3011_spo2_read(&ppg_s_dat) == 0) {
        return;
    }
    gsensor_read_cbuf();
...
static u8 sensor_init(void)
{
    if (hx3011_chip_check()) {
        ...
        if (data_buf == NULL) {
            log_e("hx3011 malloc gsensor cbuf error1!\n");
            return 0;
        }
        gsensor_cbuf = zalloc(sizeof(cbuffer_t));
        if (gsensor_cbuf == NULL) {
            free(data_buf);
            log_e("hx3011 malloc gsensor cbuf error2!\n");
            return 0;
        }
...
static int sensor_ctl(u8 cmd, void *arg)
{
    ...
    case HR_SENSOR_FACTORY_TEST:
        hx3011_init(FT_LEAK_LIGHT_MODE);
        break;

    case HR_SEARCH_SENSOR: {
        int valid = hx3011_chip_check();
        memcpy(arg, &valid, 4);
        break;
    }
