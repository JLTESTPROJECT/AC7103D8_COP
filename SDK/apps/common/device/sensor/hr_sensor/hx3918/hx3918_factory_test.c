...
void hx3918_factory_ft_leak_light_test_config(void)//漏光测试配置
{
    uint16_t sample_rate = 25;                  /*config the data rate of chip alps2_fm ,uint is Hz*/
    uint32_t prf_clk_num = 32000 / sample_rate; /*period in clk num, num = Fclk/fs */
...
void hx3918_factory_ft_int_test_config(void)//中断测试配置
{
}
void hx3918_phase_enable(void) //中断测试配置
{
    uint8_t ldrsel_ps0 = 0;
    uint8_t ldrsel_ps1 = GREEN_LED_SLE;
    uint8_t ldrsel_ps2 = RED_LED_SLE;
...
void hx3918_phase_IR_enable(void) //中断测试配置
{
    uint8_t ldrsel_ps0 = 0;
...
bool hx3918_factory_test_read(int32_t *ps_data)
{
    ...
    ps_data[2] = P3 ;
    ps_data[3] = P4 ;
    DEBUG_PRINTF("GREEN,RED,ALS  %d %d %d\r\n",  P2, P3, P4);
    return true;
}

bool hx3918_factory_test_ir_read(int32_t *ps_data)
{
    uint8_t  databuf1[6] = {0};
    uint8_t  databuf2[6] = {0};
    int32_t P1 = 0, P2 = 0, P4 = 0, P3 = 0;
    ...
    ps_data[2] = P3 ;
    ps_data[3] = P4 ;
    DEBUG_PRINTF("IR,ALS  %d %d\r\n",  P2, P4);
    return true;
}
bool hx3918_chip_check_id(void)
{
    uint8_t i = 0;
    uint8_t chip_idf = 0;
    for (i = 0; i < 10; i++) {
        ...
        DEBUG_PRINTF("hx3918_chip_check_id  %x\r\n", chip_idf);
        if (chip_idf == HX3918_ID || chip_idf == HX3917_ID) {
            return true;
        }
    }
    return false;
...
factory_result_t hx3918_factroy_test(uint32_t  test_mode)
{
    ...
    for (ii = 0; ii < 4; ii++) { //40ms read data
        ...
        green_led_data = ps_data[1] - ps_data[3];
        red_led_data = ps_data[2] - ps_data[3];
        green_buffer[ii] = green_led_data;
        red_buffer[ii] = red_led_data;
    }
    for (ii = 0; ii < 4; ii++) { //40ms read data
        hx3918_delay(40);
        hx3918_phase_IR_enable();
        hx3918_delay(40);
        hx3918_factory_test_ir_read(ps_data);
        ir_led_data = ps_data[1] - ps_data[3];
