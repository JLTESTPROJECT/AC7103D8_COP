...
//
static hx3918_hrv_wear_msg_code_t hrv_wear_status = MSG_HRV_NO_WEAR;
static hx3918_hrv_wear_msg_code_t hrv_wear_status_pre = MSG_HRV_NO_WEAR;

static uint8_t no_touch_cnt = 0;


void hx3918_hrv_ppg_init(void) //20200615 ericy ppg fs=25hz, phase3 conversion ready interupt en
{
...
void hx3917_hrv_ppg_init(void) //20200615 ericy ppg fs=25hz, phase3 conversion ready interupt en
{
    uint16_t sample_rate = 25;                  /*config the data rate of chip alps2_fm ,uint is Hz*/
    uint32_t prf_clk_num = 32000 / sample_rate; /*period in clk num, num = Fclk/fs */
    ...
    uint8_t ldrsel_ps3 = GREEN_LED_SLE;
...
hx3918_hrv_wear_msg_code_t hx3918_hrv_wear_mode_check(int32_t infrared_data)
{
    int16_t sar_data = 32767;
#ifdef SAR_ALG_LIB
    sar_data = hx90xx_get_prox_data();
#endif
    if (infrared_data > hrv_ir_wear_thres && sar_data > sar_wear_thres) {
    ...
    } else if (infrared_data < hrv_ir_unwear_thres || sar_data < sar_unwear_thres) {
        if (no_touch_cnt > 0) {
            no_touch_cnt--;
        }
        if (no_touch_cnt == 0) {
    ...
    if (hrv_wear_status_pre != hrv_wear_status) {
        ...
        //TYHX_LOG("[hx3918_hrv_wear_mode_check start] ir wear result = %d\r\n",hrv_wear_status);
        if (hrv_wear_status_pre == MSG_HRV_NO_WEAR) {
            if (hx3918_chip_id == HX3918_ID) {
                hx3918_hrv_low_power();
            } else {
                hx3917_hrv_low_power();
            }
        } else if (hrv_wear_status_pre == MSG_HRV_WEAR) {
            tyhx_hrv_alg_open_deep();
            if (hx3918_chip_id == HX3918_ID) {
                hx3918_hrv_set_mode(PPG_INIT);
                hx3918_hrv_set_mode(CAL_INIT);
            } else {
                hx3917_hrv_set_mode(PPG_INIT);
                hx3917_hrv_set_mode(CAL_INIT);
            }
        }
    }
...
void PPG_hrv_Calibration_Routine(HRV_CAL_SET_T *calR, int32_t led, int32_t amb)
{
    ...
    case hrvLedCur:
        ...
        if (led > amb + 128) {
            ...
            calR->led_cur = 5000 * (hx3918_hrs_agc_idac + calR->amb_idac) / calR->led_step;
            if (calR->led_cur > calR->led_max_cur) {
                calR->led_cur = calR->led_max_cur;
            }
            calR->led_idac = hx3918_hrs_agc_idac >> 1;
            calR->ontime = 64;
            calR->flag = CAL_FLG_LED_DR | CAL_FLG_LED_DAC | CAL_FLG_AMB_DAC | CAL_FLG_RF;
            calR->state = hrvCalLed;
        } else {
...
HRV_CAL_SET_T PPG_hrv_agc(void)
{
    ...
    if (calReg.state == hrvCalFinish) {
        ...
        if (hx3918_chip_id == HX3918_ID) {
            hx3918_hrv_updata_reg();
        } else {
            hx3917_hrv_updata_reg();
        }
#if defined(INT_MODE)
        hx3918_gpioint_cfg(true);
#endif
...
void hx3918_hrv_cal_init(void) // 20200615 ericy afe cali online
{
...
void hx3918_hrv_cal_off(void) // 20200615 ericy afe cali offline
{
    uint16_t sample_rate = 125;                      /*config the data rate of chip alps2_fm ,uint is Hz*/
    uint32_t prf_clk_num = 32000 / sample_rate;      /*period in clk num, num = Fclk/fs */
    ...
    if (hx3918_chip_id == HX3918_ID) { //this  is 3918
        hx3918_write_reg(0x3d, 0x83);     //bits<3:0> fifo data sel, 0000 = p1;0001= p2;0010=p3;0011=p4;bits<7> fifo enble
    } else { //this is 3917
        hx3918_write_reg(0x3d, 0x81);     //bits<3:0> fifo data sel, 0000 = p1;0001= p2;0010=p3;0011=p4;bits<7> fifo enble
    }
...
void hx3918_hrv_low_power(void)
{
...
void hx3917_hrv_low_power(void)
{
    uint16_t sample_rate = 5;                  /*config the data rate of chip alps2_fm ,uint is Hz*/
    uint32_t prf_clk_num = 32000 / sample_rate; /*period in clk num, num = Fclk/fs */
...
void hx3918_hrv_updata_reg(void)
{
    if (calReg.flag & CAL_FLG_LED_DR) {
        hx3918_write_reg(0X28, calReg.led_cur);
    }

    if (calReg.flag & CAL_FLG_LED_DAC) {
        hx3918_write_reg(0X2e, (uint8_t)calReg.led_idac);
    }

    if (calReg.flag & CAL_FLG_AMB_DAC) {
        hx3918_write_reg(0X2f, (uint8_t)calReg.amb_idac);
    }
...
void hx3917_hrv_updata_reg(void)
{
    if (calReg.flag & CAL_FLG_LED_DR) {
        hx3918_write_reg(0X29, calReg.led_cur);
    }

    if (calReg.flag & CAL_FLG_LED_DAC) {
        hx3918_write_reg(0X2f, (uint8_t)calReg.led_idac);
    }

    if (calReg.flag & CAL_FLG_AMB_DAC) {
        hx3918_write_reg(0X2e, (uint8_t)calReg.amb_idac);
    }
...
void hx3918_hrv_set_mode(uint8_t mode_cmd)
{
    switch (mode_cmd) {
    case PPG_INIT:
        hx3918_hrv_ppg_init();
...
#endif
#endif
        //AGC_LOG("init  %x,%x \r\n", hx3918_read_reg(0x23),hx3918_read_reg(0x24));
        s_cal_state = 1;
...
void hx3917_hrv_set_mode(uint8_t mode_cmd)
{
    switch (mode_cmd) {
    case PPG_INIT:
        hx3917_hrv_ppg_init();
...
SENSOR_ERROR_T hx3918_hrv_enable(void)
{
...
void hx3918_hrv_disable(void)
{
    hx3918_ppg_timer_cfg(false);
    hx3918_agc_timer_cfg(false);

    hx3918_hrv_set_mode(PPG_OFF);
    s_ppg_state = 0;
    s_cal_state = 0;
    tyhx_hrv_alg_close();
...
void hx3918_hrv_data_reset(void)
{
    s_ppg_state = 0;
    s_cal_state = 0;
    tyhx_hrv_alg_close();
...
hx3918_hrv_wear_msg_code_t hx3918_hrv_get_wear_status(void)
{
    return  hrv_wear_status;
}

hx3918_hrv_wear_msg_code_t hx3918_hrv_get_wear_status_pre(void)
{
...
uint8_t hx3918_hrv_read(ppg_sensor_data_t *s_dat)
{
    ...
    if (hx3918_chip_id == HX3917_ID) {
        ps_green_data = ps_data[3];
    } else {
        ps_green_data = ps_data[2];
    }

    data_count = hx3918_read_fifo_data(s_buf, 2, 1);
    ...
    for (uint8_t i = 0; i < data_count; i++) {
        ...
        hx3918_hrv_wear_mode_check(Ir_src_data);
    ...
    if (recal) {
        ...
        if (cal_delay <= 0) {
            ...
            if (hx3918_chip_id == HX3918_ID) {
                hx3918_hrv_set_mode(RECAL_INIT);
            } else {
                hx3917_hrv_set_mode(RECAL_INIT);
            }
        }
    } else {
        cal_delay = CAL_DELAY_COUNT;
