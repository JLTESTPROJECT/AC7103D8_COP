...
static hx3918_hrs_wear_msg_code_t hrs_wear_status = MSG_HRS_INIT;
static hx3918_hrs_wear_msg_code_t hrs_wear_status_pre = MSG_HRS_INIT;

static uint8_t no_touch_cnt = 0;

void hx3918_hrs_ppg_init(void) //20200615 ericy ppg fs=25hz, phase3 conversion ready interupt en
{
...
void hx3917_hrs_ppg_init(void) //20200615 ericy ppg fs=25hz, phase3 conversion ready interupt en
{
    uint16_t sample_rate = 25;                  /*config the data rate of chip alps2_fm ,uint is Hz*/
    uint32_t prf_clk_num = 32000 / sample_rate; /*period in clk num, num = Fclk/fs */
...
hx3918_hrs_wear_msg_code_t hx3918_hrs_wear_mode_check(int32_t infrared_data)
{
    ...
    if (infrared_data < check_thre || sar_data < sar_unwear_thres) {
    ...
    } else if (infrared_data >= check_thre && sar_data > sar_wear_thres) {
        if (no_touch_cnt > 0) {
            no_touch_cnt--;
        }
        if (no_touch_cnt == 0) {
    ...
    //AGC_LOG("hrs wear: ir=%d,  %x\r\n",infrared_data,hx3918_read_reg(0x6a));
    ...
    if (hrs_wear_status_pre != hrs_wear_status) {
        ...
        if (hrs_wear_status == MSG_HRS_NO_WEAR) {
            if (hx3918_chip_id == HX3918_ID) {
                hx3918_hrs_low_power();
            } else {
                hx3917_hrs_low_power();
            }
        } else if (hrs_wear_status == MSG_HRS_WEAR) {
            tyhx_hrs_alg_open_deep();
            if (hx3918_chip_id == HX3918_ID) {
                hx3918_hrs_set_mode(PPG_INIT);
                hx3918_hrs_set_mode(CAL_INIT);
            } else {
                hx3917_hrs_set_mode(PPG_INIT);
                hx3917_hrs_set_mode(CAL_INIT);
            }
        }
    }
...
HRS_CAL_SET_T PPG_hrs_agc(void)
{
    ...
    if (calReg.state == hrsCalFinish) {
        hx3918_hrs_set_mode(CAL_OFF);
    } else {
        if (hx3918_chip_id == HX3918_ID) {
            hx3918_hrs_updata_reg();
        } else {
            hx3917_hrs_updata_reg();
        }
    }
#if defined(INT_MODE)
...
void hx3918_hrs_cal_init(void) // 20200615 ericy afe cali online
{
...
void hx3918_hrs_cal_off(void) // 20200615 ericy afe cali offline
{
    uint16_t sample_rate = 25;                      /*config the data rate of chip alps2_fm ,uint is Hz*/
    uint32_t prf_clk_num = 32000 / sample_rate;      /*period in clk num, num = Fclk/fs */
...
void hx3917_hrs_updata_reg(void)
{
    ...
    if (calReg.flag & CAL_FLG_RF) {
        hx3918_write_reg(0X1e, calReg.ontime); //3917 p2
        hx3918_write_reg(0X1f, calReg.ontime); //3917 p3
    }
}
...
uint8_t hx3918_hrs_read(ppg_sensor_data_t *s_dat)
{
    ...
    } else if (hx3918_chip_id == HX3917_ID) {
...
//        DEBUG_PRINTF("ppg data size: %d\r\n", Ir_src_data);
        hx3918_hrs_wear_mode_check(Ir_src_data);
...
#ifdef SAR_ALG_LIB
    if (hrs_wear_status == MSG_HRS_NO_WEAR) {
        hx3918_cali_with_ir(Ir_src_data, hrs_ir_unwear_thres);
    }
#endif
    ...
    if (recal) {
        ...
        if (cal_delay <= 0) {
            ...
            if (hx3918_chip_id == HX3918_ID) {
                hx3918_hrs_set_mode(RECAL_INIT);
            } else {
                hx3917_hrs_set_mode(RECAL_INIT);
            }
        }
    } else {
        cal_delay = CAL_DELAY_COUNT;
