...
/* -------------------------------------------------------------------------*/
/**
 * @brief 运动算法设置放置桌面识别参数
 * @param [in] unsigned char thres[3]   输入3个参数
 * @note  正常不需要调用该接口
 */
void algo_motion_on_desk_set(unsigned char thres[3]);
 ...
 * @brief 运动手势算法获取就绪状态
 *
 * @param [out]    1：就绪   0：未就绪
 * @note  手势算法就绪时，识别将开始，需要关闭马达振动和响铃
 *
 */
short algo_motion_gesture_is_ready(void);
