...
#if OS_CPU_CORE > 1
const int CONFIG_JLSTREAM_MULTI_THREAD_ENABLE = TCFG_JLSTREAM_EFFICIENT_MODE; //音频流多线程使能
...
const int config_audio_dac_underrun_detect_enable = 1;
...
#define TCFG_LC3_PLC_TYPE  2
#endif

const int LC3_PLC_EN = TCFG_LC3_PLC_TYPE;
const char config_lib_lc3_enc_ltpf_idx_enable = ((TCFG_LC3_PLC_TYPE == 4) ? 1 : 0);   //是否编码ltpf参数,影响aplc效果. 速度要求比较高,编解码需要mips 都会增多,非APLC类型建议关掉
...
#define TCFG_JLA_PLC_TYPE  2
#endif

const  int  JLA_PLC_EN = TCFG_JLA_PLC_TYPE;           //0_fade,1_时域,2_频域,3静音;
...
#define TCFG_JLA_V2_PLC_TYPE  2
#endif

const int JLA_V2_PLC_EN = TCFG_JLA_V2_PLC_TYPE;     //pcl类型配置：0_fade,1_时域plc,2_频域plc,3补静音包;
