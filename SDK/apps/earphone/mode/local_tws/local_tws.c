...
/* 功能描述： Sink端进入Sink模式时回复Source端的接口，外部流程不会调用*/
/* 输入参数描述： void */
/* 输出参数描述： void */
...
/* 输出参数描述： 0：表示执行成功  -1：tws未连接，走原本模式的流程*/
...
static int local_tws_msg_handler(int *msg)
{
    ...
    case CMD_TWS_ENTER_SINK_MODE_REQ:
        //Sink进入sink模式
        log_info("CMD_TWS_ENTER_SINK_MODE_REQ: %d %d %d, source mode:%d\n", __this->cmd_record, __this->cmd_timestamp, cmd_timestamp, cmd[1]);
    ...
    case CMD_TWS_CONNECT_MODE_REPORT:
        log_info("CMD_TWS_CONNECT_MODE_REPORT\n");
    ...
    case CMD_TWS_ENTER_NO_SOURCE_MODE_REPORT:
        r_printf("CMD_TWS_ENTER_NO_SOURCE_MODE_REPORT\n");
        if (app_in_mode(APP_MODE_BT) && a2dp_player_runing() && (tws_api_get_role() == TWS_ROLE_MASTER)) {
