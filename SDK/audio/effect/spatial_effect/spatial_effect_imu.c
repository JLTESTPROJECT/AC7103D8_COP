...
#pragma bss_seg(".spatial_effect_imu.data.bss")
#pragma data_seg(".spatial_effect_imu.data")
#pragma const_seg(".spatial_effect_imu.text.const")
#pragma code_seg(".spatial_effect_imu.text")
#endif
/*****************************************************************
>
...
struct space_data_context {
    ...
    u16 timeout;
    u16 timer;
#if (defined TCFG_SENSOR_DATA_EXPORT_ENABLE) && (TCFG_SENSOR_DATA_EXPORT_ENABLE == SENSOR_DATA_EXPORT_USE_UART)
    u8 cbuf[1024];
    cbuffer_t data_cbuf;
...
int __icm42670p_space_motion_data_read(void *sensor, void *data, int len)
{
    ...
    int data_len = imu_sensor_io_ctl((char *)sensor_name, IMU_GET_SENSOR_READ_FIFO, (u8 *)data);
    if (data_len < 12) {
        return 0;
    }
...
void *__icm42670p_space_motion_detect_open(void)
{
    ...
    u8 arg_data;
    //唤醒MPU
    imu_sensor_io_ctl((char *)sensor_name, IMU_SENSOR_WAKEUP, &arg_data);

    //检查传感器id
    int ret = imu_sensor_io_ctl((char *)sensor_name, IMU_SENSOR_SEARCH, &arg_data);
    printf("ret : %d\n", ret);
    if (ret != 0) {
        return NULL;
    }
...
void __icm42670p_space_motion_detect_close(void *sensor)
{
    ...
    if (ctx) {
        //MPU进入睡眠
        imu_sensor_io_ctl((char *)sensor_name, IMU_SENSOR_SLEEP, NULL);
        if (ctx->timer) {
            sys_timer_del(ctx->timer);
            ctx->timer = 0;
        }
...
void *__mpu6887p_space_motion_detect_open(void)
{
    ...
    if ((status_temp & 0x01) == 0) {
        //如果还没有初始化，重新初始化
        imu_sensor_io_ctl((char *)name_test, IMU_SENSOR_ENABLE, &arg_data);
    }
    //检查传感器id
    int ret = imu_sensor_io_ctl((char *)name_test, IMU_SENSOR_SEARCH, &arg_data);
    printf("ret : %d\n", ret);
    if (ret != 0) {
        return NULL;
    }
...
int __qmi8658_space_motion_data_read(void *sensor, void *data, int len)
{
    ...
    int data_len = imu_sensor_io_ctl((char *)name_test, IMU_GET_SENSOR_READ_FIFO, (u8 *)data);
    if (data_len < 12) {
        return 0;
    }
...
void *__qmi8658_space_motion_detect_open(void)
{
    ...
    u8 arg_data;
    //唤醒MPU
    imu_sensor_io_ctl((char *)name_test, IMU_SENSOR_WAKEUP, &arg_data);
...
#if (defined TCFG_SENSOR_DATA_EXPORT_ENABLE) && (TCFG_SENSOR_DATA_EXPORT_ENABLE == SENSOR_DATA_EXPORT_USE_UART)
    cbuf_write(&ctx->data_cbuf, data, data_len);
...
void space_motion_sensor_sleep(void *sensor, u8 en)
{
    ...
    u8 sensor_name[20] = "lsm6dsl";
#elif TCFG_MPU6887P_ENABLE
    u8 sensor_name[20] = "mpu6887p";
#elif TCFG_QMI8658_ENABLE
    u8 sensor_name[20] = "qmi8658";
#else /*TCFG_MPU6050_EN*/
    u8 sensor_name[20] = "mpu6050";
#endif
    if (en) {
        imu_sensor_io_ctl((char *)sensor_name, IMU_SENSOR_SLEEP, NULL);
    } else {
        imu_sensor_io_ctl((char *)sensor_name, IMU_SENSOR_WAKEUP, NULL);
    }
