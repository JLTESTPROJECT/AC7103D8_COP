...
static u8 local_tws_data_trans_open(struct local_tws_source_context *ctx)
{
    ...
    if (local_tws_channel) {
        ...
        //码率超过u8所能表示的范围了
        if (ctx->fmt.bit_rate / 1000 > 255) {
            args[5] = 0xa5;     //起一个标识后一位是保存码率溢出的值
            args[6] = (ctx->fmt.bit_rate / 1000) / 255; //保存溢出的码率
            err = tws_api_data_trans_start(local_tws_channel, args, 7);
        } else {
            err = tws_api_data_trans_start(local_tws_channel, args, 5);
        }
#if 0
...
REGISTER_STREAM_NODE_ADAPTER(local_tws_source_adapter) = {
    ...
    .handle_frame = local_tws_source_handle_frame,
