...
#pragma bss_seg(".cvp_sms_vf_node.data.bss")
#pragma data_seg(".cvp_sms_vf_node.data")
#pragma const_seg(".cvp_sms_vf_node.text.const")
#pragma code_seg(".cvp_sms_vf_node.text")
#endif
#include "jlstream.h"
...
__CVP_BANK_CODE
void cvp_sms_vf_node_param_cfg_update(struct cvp_cfg_t *cfg, AEC_CONFIG *p)
{
    if (cfg == NULL) {
        return;
    }
    ...
    if (p) {
        ...
        audio_sms_vf_probe_param_update(&pre_cfg);

        //更新算法参数
        p->aec_mode = cfg->aec.en | (cfg->nlp.en << 1) ;
        p->ul_eq_en = 0;
...
int cvp_sms_vf_param_cfg_read(void)
{
    u8 subid;
    if (g_cvp_hdl) {
    ...
    len = jlstream_read_node_data(NODE_UUID_CVP_SMS_VF, subid, (u8 *)&ncfg);
    if (len != sizeof(ncfg)) {
...
u8 cvp_sms_vf_get_talk_mic_ch(void)
{
...
int cvp_sms_vf_node_param_cfg_read(void *priv, u8 ignore_subid, u16 algo_uuid)
{
    ...
    len = jlstream_read_node_data(NODE_UUID_CVP_SMS_VF, subid, (u8 *)&ncfg);
    if (len != sizeof(ncfg)) {
        printf("cvp_sms_aec_node read ncfg err\n");
        return 0;
    }
    ...
    cvp_sms_vf_node_param_cfg_update(&cfg, p);
...
static void cvp_handle_frame(struct stream_iport *iport, struct stream_note *note)
{
    ...
    while (1) {
        ...
        if (hdl->ref_mic.en) { //参考数据硬回采
            ...
            audio_sms_vf_spk_data_push(ref_buf, NULL, wlen << hdl->ref_mic_num);
            audio_sms_vf_talk_mic_push(mic_buf, wlen << 1);
            hdl->unlock();
        } else {//参考数据软回采
            hdl->lock();
            ...
            audio_sms_vf_talk_mic_push(dat, in_frame->len);
            hdl->unlock();
            if (++hdl->buf_cnt > ((CVP_INPUT_SIZE / 256) - 1)) {
                hdl->buf_cnt = 0;
            }
...
static int cvp_sms_vf_lock_init(struct cvp_node_hdl *hdl)
{
    int ret = false;
    ...
    if (hdl) {
        ...
        case  NODE_UUID_CVP_SMS_VF:
            hdl->lock   = audio_cvp_sms_vf_tde_lock;
            hdl->unlock = audio_cvp_sms_vf_tde_unlock;
            break;
        default:
            ASSERT(0, "cvp_sms_vf_lock_init: unknown node UUID\n");
            break;
        }
        ret = true;
    }
...
static int cvp_adapter_bind(struct stream_node *node, u16 uuid)
{
    ...
    /*初始化spinlock*/
    cvp_sms_vf_lock_init(hdl);
    node->type = NODE_TYPE_ASYNC;
    g_cvp_hdl = hdl;
    /*先读取节点需要用的参数*/
    cvp_sms_vf_node_param_cfg_read(NULL, 0, uuid);
...
static int cvp_ioc_update_parm(struct cvp_node_hdl *hdl, int parm)
{
    ...
    if (hdl) {
        cvp_sms_vf_node_param_cfg_update(cfg, &hdl->online_cfg);
        sms_vf_tde_cfg_update(&hdl->online_cfg);
        ret = true;
    }
    return ret;
}
...
//注册工具在线调试
REGISTER_ONLINE_ADJUST_TARGET(cvp_sms_vf) = {
    .uuid       = NODE_UUID_CVP_SMS_VF,
};

#endif/*TCFG_AUDIO_CVP_SMS_AEC*/
