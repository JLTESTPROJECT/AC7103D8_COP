...
static int anc_mult_scene_set_base(u16 scene_id)
{
    ...
    if (ret == 0) {
        //找到对应场景的滤波器，对FF/FB 滤波器参数赋值
        if (anc_mult_cfg_analsis(&bulk->lff, &mult_hdl->lff_coeff, &param->gains.l_ffgain, audio_anc_gains_alogm_get(ANC_FF_TYPE)) > 0) {
        ...
        if (anc_mult_cfg_analsis(&bulk->lfb, &mult_hdl->lfb_coeff, &param->gains.l_fbgain, audio_anc_gains_alogm_get(ANC_FF_TYPE)) > 0) {
    ...
    if (cmp_bulk) {
        //找到对应场景的滤波器，对CMP 滤波器参数赋值
        if (anc_mult_cfg_analsis(&cmp_bulk->lcmp, &mult_hdl->lcmp_coeff, &param->gains.l_cmpgain, audio_anc_gains_alogm_get(ANC_CMP_TYPE)) > 0) {
...
//单独增益ID设置-兼容之前的命令
void audio_anc_mult_gains_id_set(u8 gain_id, void *buf, int len)
{
    if (!mult_hdl) {
        return;
    }
    ...
    case ANC_GAIN_SIGN_SET:
        ...
        for (i = 0; i < 8; i++) {
            ...
            int gain_sign = *((int *)buf);
            if (((gain_sign & BIT(i)) && (p->gains->iir_gain > 0)) ||  \
                ((!(gain_sign & BIT(i))) && (p->gains->iir_gain < 0))) {
...
int audio_anc_mult_gains_id_get(u8 gain_id, int *data)
{
    ...
    switch (gain_id) {
    case ANC_FFGAIN_SET:
        gains = bulk->lff.gains;
        goto __get_gain;
    case ANC_FBGAIN_SET:
        gains = bulk->lfb.gains;
        goto __get_gain;
    case ANC_TRANS_GAIN_SET:
        gains = bulk->ltrans.gains;
        goto __get_gain;
    case ANC_CMP_GAIN_SET:
        gains = bulk->lcmp.gains;
        goto __get_gain;
    case ANC_RFF_GAIN_SET:
        gains = bulk->rff.gains;
        goto __get_gain;
    case ANC_RFB_GAIN_SET:
        gains = bulk->rfb.gains;
        goto __get_gain;
    case ANC_RTRANS_GAIN_SET:
        gains = bulk->rtrans.gains;
        goto __get_gain;
    case ANC_RCMP_GAIN_SET:
        gains = bulk->rcmp.gains;
        goto __get_gain;
    case ANC_GAIN_SIGN_SET:
        anc_mult_param_t *p;
        *data = 0;
...
__get_gain:
    ...
    //获取绝对值
    if (gains) {
        abs_gains = (gains->iir_gain > 0) ? gains->iir_gain : 0 - gains->iir_gain;
        memcpy((u8 *)data, (u8 *)&abs_gains, sizeof(float));
    } else {
        ret = -1;
    }
    anc_mult_log("scene_id %d get_gain, %d/100 ", bulk->scene_id, (int)(abs_gains * 100));
    return ret;
}
